...
class MapsFragment: Fragment() {
    private lateinit var googleMap: GoogleMap // создаем сылку на GoogleMap
    private var myMarkers = mutableListOf<MarkerOptions>()
    private val markerInfo = MarkerBottomSheetFragment()
    ...
    private val requestPermissionLauncher =
        registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted -> // проверяем предоставленно ли разршение
            if (isGranted) { // если предоставленно
                googleMap.apply {// устанавливаем в обьект рахзрешения
                    isMyLocationEnabled = true
                    uiSettings.isMyLocationButtonEnabled = true // будет отресован занчек поиска локации// вызываем пользовательский интерфейс googleMaps
                }
            } else {
                Toast.makeText(
                    requireContext(),
                    "Без доступа приложение не буде полноцено функционировать",
                    Toast.LENGTH_LONG
                ).show()
            }
        }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val mapFragment = childFragmentManager.findFragmentById(R.id.map) as SupportMapFragment // создаем фрагмен

        loadMarkers()

        lifecycle.coroutineScope.launchWhenCreated { // запускаем корутину
            ...
            googleMap.setOnMarkerClickListener { marker ->
                ...
                if (showMode) {
                    ...
                    markerInfo.arguments = args
                    markerInfo.show(childFragmentManager, "mark")
                }
    ...
    private fun loadMarkers() {
        val founderListType: Type =
            object : TypeToken<ArrayList<Marker>>() {}.type
        requireActivity().getPreferences(Context.MODE_PRIVATE)?.let { sp ->
            sp.getString("markers", "[]")?.let { list ->
                val prepList: ArrayList<ru.netology.map.Marker> = GsonBuilder()
                    .create()
